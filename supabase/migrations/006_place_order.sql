-- ============================================================
-- Delivr Platform — Migration 006
-- Guest order placement. Every price is looked up server-side,
-- so a tampered client cannot change what an order costs.
-- ============================================================
...
BEGIN
  ...
  ELSE
    SELECT NULL::UUID AS zone_id, true AS is_match, 0::NUMERIC AS fee,
           coalesce(s.min_order_amount, 0) AS min_amount, 0 AS extra_min,
           NULL::DOUBLE PRECISION AS dist_km, NULL::NUMERIC AS free_above,
           'pickup'::TEXT AS match_type INTO mz;
  END IF;
...
GRANT EXECUTE ON FUNCTION delivr_place_order(TEXT, UUID, TEXT, JSONB, JSONB, TEXT, TIMESTAMPTZ, TEXT, TEXT)
