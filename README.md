...
| **Πελάτης** | `/store/SLUG` | Ο δικός του σύνδεσμος ενός καταστήματος: μενού, delivery με διεύθυνση που γράφει ο πελάτης, ή take away. Χωρίς κατάλυμα, χωρίς εγγραφή. |
...
## Κατάστημα χωρίς δικά μας καταλύματα

Το ίδιο σύστημα πουλιέται και σε μεμονωμένο μαγαζί. Στο `/admin` → Καταστήματα →
καρτέλα **«Δικός του σύνδεσμος»** ενεργοποιείς το `standalone_enabled` και παίρνεις
έτοιμο σύνδεσμο `{app_url}/store/{slug}` με QR.

Τι αλλάζει για τον πελάτη: δεν υπάρχει κατάλυμα, άρα **διαλέγει ο ίδιος περιοχή**
από τις ζώνες του καταστήματος και γράφει διεύθυνση, όροφο, κουδούνι και οδηγίες
για τον διανομέα. Η περιοχή καθορίζει μεταφορικά και ελάχιστη παραγγελία. Επειδή
δεν υπάρχουν συντεταγμένες, **δεν υπάρχει fallback ακτίνας**: ή ταιριάζει ζώνη, ή
η παραγγελία απορρίπτεται με `OUT_OF_RANGE`. Κατάστημα χωρίς καμία ζώνη δέχεται
κάθε περιοχή με τα βασικά του μεταφορικά, ώστε να μπορεί να ξεκινήσει αμέσως.

Η διεύθυνση αποθηκεύεται **στη συσκευή του πελάτη** (localStorage) για την επόμενη
φορά — ποτέ σε αναζητήσιμο πίνακα ανά τηλέφωνο.

Στη χρέωση δεν αλλάζει τίποτα: η προμήθεια ή το πάγιο του καταστήματος χρεώνονται
κανονικά και το `orders.channel` γράφει `store` αντί για `qr`, ώστε να ξεχωρίζει
ο τζίρος στα στατιστικά.
